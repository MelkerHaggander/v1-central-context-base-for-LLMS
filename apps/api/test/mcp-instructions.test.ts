...
describe("MCP-instruktioner V1.2", () => {
  ...
  it("kräver ett get_context-anrop med hela prompten", () => {
    ...
    assert.match(searchFirst, /where the memory landed/);
  ...
  it("beskriver ett save_memory när arbetet är klart", () => {
    ...
    assert.match(MEMORY_INSTRUCTIONS, /not stored raw/);
  ...
  it("kräver proaktiv användning och radering bara i dashboarden", () => {
    assert.match(MEMORY_INSTRUCTIONS, /Use memory proactively and frequently/);
    assert.match(MEMORY_INSTRUCTIONS, /Never delete memories/);
    assert.match(MEMORY_INSTRUCTIONS, /only in the dashboard/);
    assert.match(MEMORY_INSTRUCTIONS, /Do not send space_id/);
  });
